    ...
    for comment in comments:
        try:
            comment_data = {
                ...
                "date": (
                    comment.created if hasattr(comment, "created") else "Unknown date"
                ),
                "author": (
                    comment.author.displayName
                    if hasattr(comment, "author")
                    and hasattr(comment.author, "displayName")
                    else "Unknown author"
                ),
            }
...
def generate_rundown(jira, issue_data, format_file=None):
    ...
    if not should_use_gemini():
        typer.echo(
            colorize(
                "❌ This feature requires Gemini AI due to large context window requirements.",
                "neg",
            )
        )
        typer.echo(
            colorize(
                "💡 Local Ollama models don't support the required context size.",
                "info",
            )
        )
    ...
    try:
        ...
        typer.echo(
            colorize(f"📝 Analyzing issue {issue_key} for progress summary...", "info")
        )
        ...
        summary_response = _execute_with_retry(
            generate_summary, "generating issue summary"
        )
